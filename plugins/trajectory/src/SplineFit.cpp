...
namespace LidarTrajectory {
  ...
  SplineFit<N>::position(double t)  const {
  ...
  template <int N>
  Eigen::Matrix<double, N, 1>
  SplineFit<N>::position(double t, Eigen::Matrix<double, N, 1>& velocity)
    const {
    auto tcomb = tconvert(t);
    int i = tcomb.first;
    double tf = tcomb.second;
  ...
  template <int N>
  Eigen::Matrix<double, N, 1>
  SplineFit<N>::position(double t,
                         Eigen::Matrix<double, N, 1>& velocity,
                         Eigen::Matrix<double, N, 1>& acceleration)
    const {
    auto tcomb = tconvert(t);
    int i = tcomb.first;
    double tf = tcomb.second;
