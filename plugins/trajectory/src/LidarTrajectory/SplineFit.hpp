#if !defined(LIDARTRAJECTORY_SPLINEFIT_HPP)
#define LIDARTRAJECTORY_SPLINEFIT_HPP 1
...
namespace LidarTrajectory {
  ...
  public:
    ...
    , v(num+1)
    , missing(num+1)            // missing data at this node?
    {}
  ...
  private:
    const double _scale;
  public:
    AccelJumpConstraint(double tblock = 1)
      : _scale(2/(tblock * tblock))
    {}
    template <typename T>
    bool operator()(const T* const ra, // N vec for pos at beg
      ...
      for (int i = 0; i < N; ++i)
        residual[i] = scale *
          (3.0 * (rc[i] - ra[i]) - (vc[i] + va[i]) - 4.0 * vb[i]) ;
      return true;
    }
  };
