...
#include <limits>
#include <Eigen/Dense>
#include <LidarTrajectory/Utils.hpp>
...
namespace LidarTrajectory {
  ...
  public:
    ...
    Pulse(double _t,
          const Eigen::Vector3d& _f, const Eigen::Vector3d& _l,
          double _ang = 0);
    bool operator<(const Pulse& p) const { return t < p.t; }
    bool MultiReturn() const { return d > 0; }
  };
  ...
  public:
    FirstLastError(const Pulse& p, double t)
      ...
      , _r(p.r)
      , _M(Utils::PerpProjector(p.n, p.d))
    {}
    template <typename T>
